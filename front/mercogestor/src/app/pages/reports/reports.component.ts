...
import { ApiService, Produto, Movimento as ApiMov } from '../../core/api.service';

type TipoFiltro = 'all' | 'in' | 'out';
type Agrupar = 'day' | 'month';

// Movimentação normalizada para o relatório
interface MovRow extends ApiMov {
  dataISO: string;          // sempre preenchido (data/dataISO/date)
  precoUnitario?: number;   // opcional
  validadeLote?: string;    // opcional (ISO)
...
export class ReportsComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  produtoId = signal<string>('');          // id do produto selecionado (string)
  ...
  lista = computed<MovRow[]>(() => {
    const t = this.tipo();
    const d0 = this.de();
    const d1 = this.ate();
    const pid = this.produtoId();

    return this._movs().filter(m => {
      ...
      if (pid && String(m.productId) !== pid) return false;
  ...
  // KPIs (quantidades)
  kEntradas = computed(() =>
    this.lista().filter(m => m.tipo === 'in').reduce((s, m) => s + m.quantidade, 0)
  );
  kSaidas = computed(() =>
    this.lista().filter(m => m.tipo === 'out').reduce((s, m) => s + m.quantidade, 0)
  );
  ...
  constructor() {
    // atualiza os gráficos sempre que filtros ou dados mudarem
    effect(() => {
  ...
  private carregarMovs() {
    this.api.listMovements().subscribe({
      next: ms => {
        const norm: MovRow[] = ms.map(m => {
          const anyM = m as any;
          let iso =
            anyM.dataISO ??
            anyM.data ??
          ...
          return {
            ...m,
            dataISO: iso,
            precoUnitario: anyM.precoUnitario ?? undefined,
            validadeLote: anyM.validadeLote ?? anyM.validade ?? undefined,
            motivo: anyM.motivo ?? anyM.reason ?? undefined
          };
        });
        this._movs.set(norm);
      },
  ...
  private createCharts() {
    ...
    const gIn = tctx.createLinearGradient(0, 0, 0, 260);
    gIn.addColorStop(0, accent + '55');
    gIn.addColorStop(1, '#ffffff00');
    const gOut = tctx.createLinearGradient(0, 0, 0, 260);
    gOut.addColorStop(0, accent2 + '55');
    gOut.addColorStop(1, '#ffffff00');
        ...
        datasets: [
          {
            label: 'Saídas',
            data: [],
            backgroundColor: [accent, accent, accent, accent2, accent2, accent2]
          }
        ]
      },
      options: {
  ...
  private updateCharts() {
    ...
    for (const k of spanKeys) {
      labels.push(this.prettyKey(k, group));
      mapIn.set(k, 0);
      mapOut.set(k, 0);
    }
    ...
    for (const m of this.lista()) {
      ...
      if (m.tipo === 'in') {
        mapIn.set(k, (mapIn.get(k) ?? 0) + m.quantidade);
      }
      if (m.tipo === 'out') {
        mapOut.set(k, (mapOut.get(k) ?? 0) + m.quantidade);
      }
    }
    ...
    const labelsBar = top.map(([pid]) =>
      this._produtos().find(p => String(p.id) === pid)?.nome ?? '—'
    );
  ...
  private buildSpanKeys(g: Agrupar, ymdStart: string, ymdEnd: string) {
    const keys: string[] = [];
    const a = new Date(ymdStart + 'T00:00:00');
    const b = new Date(ymdEnd + 'T00:00:00');
    if (g === 'day') {
  ...
  exportDetalhado() {
    ...
    for (const m of this.lista()) {
      const prodName =
        this._produtos().find(x => String(x.id) === String(m.productId))?.nome ?? '—';
      rows.push([
  ...
  exportResumido() {
    ...
    for (const k of keys) {
      const e = mapIn.get(k) ?? 0;
      const s = mapOut.get(k) ?? 0;
  ...
  // nome do produto por id (aceita string | number)
  nomeDe(id: string | number | undefined) {
    const key = String(id ?? '');
    if (!key) return '—';
